...
import { Plus, Search, Pill, Edit } from 'lucide-react';
...
const StaffMedicines = () => {
  ...
  const [form, setForm] = useState({ name: '', category: '', price: '', stock_quantity: '', low_stock_threshold: '10', expiry_date: '', description: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { ...form, price: parseFloat(form.price), stock_quantity: parseInt(form.stock_quantity), low_stock_threshold: parseInt(form.low_stock_threshold), pharmacy_id: pharmacyId, created_by: user?.id };
    ...
    setIsOpen(false); setEditingMed(null); setForm({ name: '', category: '', price: '', stock_quantity: '', low_stock_threshold: '10', expiry_date: '', description: '' });
    fetchMedicines();
  };

  const openEdit = (med: any) => {
    setEditingMed(med);
    setForm({ name: med.name, category: med.category || '', price: med.price.toString(), stock_quantity: med.stock_quantity.toString(), low_stock_threshold: med.low_stock_threshold.toString(), expiry_date: med.expiry_date || '', description: med.description || '' });
    setIsOpen(true);
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map(med => (
          <Card key={med.id} className="card-hover">
            <CardContent className="p-4">
              <div className="flex justify-between items-start">
                ...
                <Button variant="ghost" size="icon" onClick={() => openEdit(med)}><Edit className="w-4 h-4" /></Button>
              </div>
              <div className="mt-4 flex justify-between items-center">
                <span className="text-lg font-bold">${med.price}</span>
                <Badge variant={med.stock_quantity <= med.low_stock_threshold ? 'destructive' : 'secondary'}>{med.stock_quantity} in stock</Badge>
              </div>
              {med.expiry_date && <p className="text-xs text-muted-foreground mt-2">Expires: {new Date(med.expiry_date).toLocaleDateString()}</p>}
            </CardContent>
          </Card>
        ))}
      </div>
